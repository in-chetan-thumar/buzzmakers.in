            ...
            <div class="accordion-item" style="display: none">
                <h2 class="accordion-header" id="heading{{$data->id}}">
                    <button class="accordion-button {{($collapsed) ? ('') : ('collapsed')}}" type="button" data-bs-toggle="collapse"
                        data-bs-target="#collapse{{$data->id}}" aria-expanded="{{($collapsed) ? ('true') : ('false')}}" aria-controls="collapse{{$data->id}}">
                        {{$data->title}}
                    </button>
                </h2>
                <div id="collapse{{$data->id}}" class="accordion-collapse collapse {{($collapsed) ? ('show') : ('')}}" aria-labelledby="heading{{$data->id}}"
